...
const Comments = ({ route }) => {
  const {category} = route.params; // Get product id from navigation params
  const {post} = route.params; // Get product id from navigation params
  ...
  const [noComments, setNoComments] = useState(false); // State to manage no comments
    ...
    const fetchComments = async () => {
      try {
        const response = await axios.post(
          'https://intechsol.co/sav/api/post_comment',
          { post_id: id },
          {
            headers: {
    ...
    // Fetch comments when component mounts or when post ID changes
    useEffect(() => {
      fetchComments();
    }, [id, apiToken]); // Ensure useEffect only runs when id or apiToken changes
    ...
    const postComment = async () => {
      ...
      try {
        const response = await axios.post(
          'https://intechsol.co/sav/api/store_comment',
