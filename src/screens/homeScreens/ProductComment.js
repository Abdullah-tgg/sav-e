...
const ProductComment = ({ route }) => {
  const { id } = route.params; // Get product id from navigation params
  ...
  useEffect(() => {
    const fetchComments = async () => {
        try {
            const response = await axios.post(
              'https://intechsol.co/sav/api/product_comments',
              { product_id: id },
              {
                headers: {
    ...
    fetchComments();
  }, [id, apiToken]); // Depend on id and apiToken
  ...
  const postComment = async () => {
    ...
    try {
      const response = await axios.post(
        'https://intechsol.co/sav/api/product_comment',
        { product_id: id, comment: comment },
        {
          headers: {
            Authorization: `Bearer ${apiToken}`,
          },
